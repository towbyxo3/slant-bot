...
class Network(commands.Cog):
    ...
    async def network(self, ctx, member: discord.Member = None):
        ...
        embed.set_footer(text=f"Displays the most frequent chat encounters of {member.name}.\nThe numbers on the graph indicate the number of times that {member.name} and the other person chatted within the same 5-minute time frame.")
